    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <div className="p-7">
        ...
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl font-bold text-gray-500 hover:bg-hover-bg/80 hover:shadow-sm hover:-translate-y-0.5 transition-all duration-300 cursor-pointer">
            {cancelText}
          </button>
          <button onClick={() => { onConfirm(); onClose(); }} className={`px-5 py-2.5 rounded-xl font-bold text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 cursor-pointer flex items-center gap-2 border border-white/20 ${isDestructive ? 'bg-red-500/95 backdrop-blur-md hover:bg-red-600 shadow-red-500/30' : 'bg-accent/95 backdrop-blur-md hover:bg-accent-hover shadow-accent/30'}`}>
