...
export const Header: React.FC<HeaderProps> = ({ isSidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <div className="header-controls flex items-center p-2.5 bg-panel-bg/85 backdrop-blur-xl border-b border-white/20 dark:border-white/10 gap-4 shrink-0 justify-between z-20 shadow-md">
      ...
      {!hideAi && appMode === 'identify' && keyData ? (
        <button onClick={() => setAiPanelVisible(true)} disabled={isAiPanelVisible} title={isAiPanelVisible ? undefined : t('assistant')} aria-hidden={isAiPanelVisible} className={`p-2 rounded-full transition-all duration-300 ${isAiPanelVisible ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-90 hover:opacity-100 hover:bg-hover-bg/80 hover:scale-105 cursor-pointer shadow-sm border border-transparent hover:border-white/20 hover:shadow-md'}`}>
