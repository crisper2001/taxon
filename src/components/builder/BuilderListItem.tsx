  ...
  return (
    ...
    >
      ...
      {hasChildren && onToggleCollapse && (
        <button onClick={onToggleCollapse} className={`w-7 h-7 flex items-center justify-center rounded-md cursor-pointer absolute z-20 transition-colors ${isSelected ? 'text-accent hover:bg-accent/10' : 'text-gray-500 hover:bg-black/10 dark:hover:bg-white/10'}`} style={{ left: `calc(${depth * 1.5}rem)` }}>
          <Icon name="ChevronRight" size={16} className={`transition-transform duration-200 ${!isCollapsed ? 'rotate-90' : ''}`} />
        </button>
      )}
      {imageUrl ? (
        <img src={imageUrl} alt={name} className={`object-cover rounded-lg shadow-sm shrink-0 ${stateIndicator ? 'w-8 h-8' : 'w-10 h-10'}`} />
      ) : (
        <div className={`bg-header-bg/80 rounded-lg shadow-sm shrink-0 flex items-center justify-center text-gray-400 ${stateIndicator ? 'w-8 h-8' : 'w-10 h-10'}`}>
          {stateIndicator ? <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isSelected ? 'bg-accent' : 'bg-text opacity-50'}`}></span> : <Icon name={iconName} size={20} className={`shrink-0 ${isSelected ? 'opacity-100 text-accent' : 'opacity-60'}`} />}
        </div>
      )}
      <span className="truncate flex-1 text-sm font-medium">{name}{badges}</span>
