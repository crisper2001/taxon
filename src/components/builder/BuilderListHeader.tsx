  ...
  return (
    <div className="p-3.5 border-b border-black/5 dark:border-white/5 flex flex-wrap justify-between items-center bg-header-bg/85 backdrop-blur-md shadow-sm shrink-0 z-10 gap-2">
      <div className="panel-title font-bold flex items-center gap-2 text-lg tracking-tight min-w-0 pr-2">
        ...
        <span className="truncate text-accent">{title}</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          <input
            ...
            className={`[&::-webkit-search-cancel-button]:hidden transition-all duration-300 ease-in-out border-none bg-transparent outline-none text-sm p-0 ${matchCount > 0 || searchTerm ? 'w-24 sm:w-32 opacity-100' : 'w-0 opacity-0 group-hover:w-32 group-hover:opacity-100 focus:w-32 focus:opacity-100'}`}
          />
        ...
        <button onClick={onAdd} className="shrink-0 px-3 py-1.5 bg-accent/95 backdrop-blur-md border border-white/20 text-white rounded-lg hover:bg-accent-hover transition-all duration-300 text-sm font-bold shadow-md hover:shadow-lg flex items-center gap-1 cursor-pointer" title={addTitle}><Icon name="Plus" size={14} /> <span className="hidden sm:inline">{addLabel}</span></button>
      </div>
    </div>
  );
};