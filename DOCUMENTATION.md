...
* **FR8.3:** The system must allow users to establish parent-child hierarchical relationships for entities and features, and move individual states between different categorical features, via visual drag-and-drop interaction (supporting both desktop mouse events and mobile long-press touch gestures with custom visual ghost elements that track finger position), featuring visual depth indicators and collapsible groups. These builder lists must be heavily memoized and flattened to maintain `O(N)` rendering performance and support massive datasets without UI freezing.
...
* **FR8.9:** The system must support exporting the constructed key data into a custom JSON format. The system must also allow users to seamlessly convert imported legacy Lucid keys into this native format.
